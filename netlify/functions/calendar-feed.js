...
function generateICalendar(bookings) {
  ...
  bookings.forEach(booking => {
    const startDateFormatted = formatICalDate(booking.startDate, false);
    const endDateFormatted = formatICalDate(booking.endDate, true); // Add 1 day for iCal DTEND (exclusive)
    const created = timestamp;
    const uid = `booking-${booking.id}@executivevacations.net`;

    console.log(`  Booking: ${booking.customerName}`);
    console.log(`    Start: ${booking.startDate} → iCal: ${startDateFormatted}`);
    console.log(`    End: ${booking.endDate} → iCal: ${endDateFormatted} (added 1 day for DTEND)`);
